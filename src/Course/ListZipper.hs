...
findLeft f (ListZipper (l:.ls) m r)
    | f l = IsZ (ListZipper ls l (m:.r))
    | otherwise = findLeft f (ListZipper ls l (m:.r))
...
findRight f (ListZipper l m (r:.rs))
    | f r = IsZ (ListZipper (m:.l) r rs)
    | otherwise = findRight f (ListZipper (m:.l) r rs)
