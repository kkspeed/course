...
-- >>> runState ((const $ put 2) =<< put 1) 02
-- ((),2)
instance Bind (State s) where
  f =<< s = State $ \x -> let (v, st) = runState s x
                          in  runState (f v) st
...
exec :: State s a -> s -> s
exec st init = snd $ runState st init

-- | Run the `State` seeded with `s` and retrieve the resulting value.
--
-- prop> \(Fun _ f) -> eval (State f) s == fst (runState (State f) s)
eval :: State s a -> s -> a
eval st init = fst $ runState st init
